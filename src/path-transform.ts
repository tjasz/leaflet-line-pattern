...
export function rotate(p: SvgPath, dtheta: number): SvgPath {
  ...
  if (p.some(c => !c.isAbsolute || c.operator === "H" || c.operator === "V")) {
    ...
    if (c.isAbsolute) {
      switch (c.operator) {
        ...
        case "H":
          ...
          commands.push({ isAbsolute: true, operator: CommandOperator.Line, parameters: c.parameters.map(x => [x, marker.y]).flat() });
          break;
        case "V":
          marker = { x: marker.x, y: c.parameters[c.parameters.length - 1] };
          commands.push({ isAbsolute: true, operator: CommandOperator.Line, parameters: c.parameters.map(y => [marker.x, y]).flat() });
          commands.push(c);
          break;
        case "Z":
          marker = subpathStart;
      ...
      switch (c.operator) {
        case "M":
          for (let j = 1; j < c.parameters.length; j += 2) {
            ...
            subpathStart = marker;
            commands.push({ isAbsolute: true, operator: c.operator, parameters: [marker.x, marker.y] })
          }
          break;
        case "L":
        case "T":
